...
class AddToJobLogForm(forms.ModelForm):
    ...
    def clean_current_tool(self):
        ...
        if not self.instance.pk and tool and tool.is_in_use:
            raise ValidationError(f"Tool {tool.code} already in use!")
        return tool
    ...
    def clean_current_machine(self):
        ...
        if not self.instance.pk and machine and machine.is_in_use:
            raise ValidationError(f"Machine {machine.machine_number} already in use!")
        return machine
    ...
    def clean(self):
        ...
        if job and tool:
            if tool not in job.tools.all():
                raise ValidationError(f"Tool {tool.code} is not part of order {job.job_code}!")
        return cleaned_data
